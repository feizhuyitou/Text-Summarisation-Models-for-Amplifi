...
def get_labels(trans_model, attn_model, iterator):
    trans_model.eval()
    attn_model.eval()
    labels = []
    trans = []
    seq = []
    with torch.no_grad():
        for src_,out_ in iterator:
        # output = [trg_len, batch_size, output_dim]
            for idx in tqdm(range(len(src_))):
                src = src_[idx].cuda()
                out = out_[idx].cuda()
                transformer_predict = transformer_greedy_decoder(trans_model, src.view(1, -1),
                                                                 start_symbol=tgt_vocab["<SoS>"])  # turn off teacher forcing
                trans.append(transformer_predict.view(-1).tolist())
                transformer_predict = ' '.join([idx2word[n.item()] for n in transformer_predict.squeeze()])
                seq2seq_predict = attn_seq2seq_greedy_decoder(attn_model, src.view(-1, 1),
                                                      start_symbol=tgt_vocab["<SoS>"])  # turn off teacher forcing
                seq.append(seq2seq_predict.view(-1).tolist())
                seq2seq_predict = ' '.join([idx2word[n.item()] for n in seq2seq_predict.squeeze()])
                truth = ' '.join(['<SoS>'] + [idx2word[n.item()] for n in out.squeeze() if n != 1])
                trans_ROUGE = rouge(transformer_predict, truth)[1]['r']
                seq2seq_ROUGE = rouge(seq2seq_predict, truth)[1]['r']
                if trans_ROUGE >= seq2seq_ROUGE:
                    labels.append(0)
                else:
                    labels.append(1)

    return labels, trans, seq



def main():
    print('Start processing the raw data')
    iterator = clean(content, src_vocab, tgt, tgt_vocab, idx)
    print('Start analysing the labels')
    labels, trans, seq = get_labels(trans_model, attn_model, iterator)
    idx_data['labels'] = labels
    idx_data['trans'] = trans
    idx_data['seq'] = seq
    idx_data.to_csv('classification_task_cleaned.csv',index=False)
    print('complete!')
...
if __name__ == "__main__":
    ...
    idx_data = pd.read_csv('classification_task.csv')
