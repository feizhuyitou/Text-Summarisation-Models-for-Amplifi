...
# test_ROUGE = evaluate(model, dev_loader)
#
...
#
# for i in range(len(enc_inputs)):
#     greedy_dec_input = greedy_decoder(model, enc_inputs[i].view(1, -1), start_symbol=tgt_vocab["<SoS>"]).cuda()
#     predict, _, _, _ = model(enc_inputs[i].view(1, -1), greedy_dec_input)
#     predict = predict.cuda()
#     predict = predict.data.max(1, keepdim=True)[1]
#     print(predict)
#     print([idx2cont[j] for j in enc_inputs[i].cpu().numpy()], '->', [idx2word[n.item()] for n in predict.squeeze()])
#     # print(enc_inputs[i], '->', [idx2word[n.item()] for n in predict.squeeze()])
#######################################################################################
##########################################################################################
