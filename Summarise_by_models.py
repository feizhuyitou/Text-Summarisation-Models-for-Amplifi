...
def final_translate(iterator, trans_model, attn_model,trg = True):
    ...
    if trg:
        ...
        for i in range(len(enc_inputs)):
            ...
            if 0 in transformer_predict or 512 in transformer_predict:
                predict = attn_seq2seq_greedy_decoder(attn_model, enc_inputs[i].view(-1, 1),
                                                      start_symbol=tgt_vocab["<SoS>"]).cuda()
                print('\n\n Attention based model Summary: ', ' '.join([idx2word[n.item()] for n in predict.squeeze()]),
                      '\n')
...
def main():
    ...
    translate(iterator, trans_model, attn_model, seq2seq,
                trg, transformer, attention, traditional)
...
if __name__ == "__main__":
    ...
    from Traditional_Seq2Seq import Encoder, Decoder, Seq2Seq
    INPUT_DIM = len(src_vocab)
    OUTPUT_DIM = len(tgt_vocab)
    ENC_EMB_DIM = 512
    DEC_EMB_DIM = 512
    ...
    # seq2seq.load_state_dict(torch.load('Seq_default.pt'))
    seq2seq.load_state_dict(torch.load('Seq_refined.pt'))
